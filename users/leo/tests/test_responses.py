...
def test_response_failure_from_empty_invalid_request():
    response = build_response_from_invalid_request(PostsSearchInvalidRequest())
...
def test_response_failure_from_invalid_request_with_errors():
    request = PostsSearchInvalidRequest()
    request.add_error("some", "error")
    request.add_error("another", "error")
