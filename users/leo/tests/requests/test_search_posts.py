...
@pytest.mark.parametrize("key", ["keyword_search", "semantic_search"])
...
def test_build_semantic_search_request_without_parameters():
    request = build_semantic_search_request()
    assert request.filters is None
    assert bool(request) is True


def test_build_semantic_search_request_with_empty_filters():
    request = build_semantic_search_request({})
    assert request.filters == {}
    assert bool(request) is True


def test_build_semantic_search_request_with_invalid_filters_parameter():
    request = build_semantic_search_request(filters="invalid")
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
    assert bool(request) is False


def test_build_semantic_search_request_with_incorrect_filter_keys():
    request = build_semantic_search_request(filters={"invalid": "value"})
    assert request.has_errors()
    assert request.errors[0]["parameter"] == "filters"
    assert bool(request) is False


@pytest.mark.parametrize("key", ["text", "model_id", "threshold"])
def test_build_semantic_search_request_with_valid_filters(key):
    filters = {key: "value"}
    request = build_semantic_search_request(filters=filters)
    assert request.filters == filters
    assert bool(request) is True
